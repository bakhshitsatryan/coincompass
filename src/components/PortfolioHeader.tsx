...
import { Compass, Wallet, Dices, SlidersHorizontal, RotateCcw } from 'lucide-react'
...
const SOURCE_META: Record<PortfolioState['source'], { label: string; icon: LucideIcon }> = {
...
export default function PortfolioHeader({ portfolio, onAmount, onReset }: Props) {
    ...
    >
      ...
      <div className={styles.value}>
        <div className={styles.valueMeta}>
          ...
          {showPl && (
            <span className={`${styles.pl} ${pl! > 0 ? styles.up : styles.down}`}>
      ...
      <div className={styles.controls}>
        <AmountOverride value={amount} currencySymbol={currencySymbol(currency) || '$'} onChange={onAmount} />
        <Button variant="ghost" size="md" leadingIcon={RotateCcw} onClick={onReset} className={styles.reset}>
          Start over
        </Button>
      </div>
    </motion.header>
