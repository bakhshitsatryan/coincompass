...
export default function ApiKeyGate({ onEnter }: Props) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      onEnter({ amount: res.totalValue || 0, currency, source: 'portfolio', raw: res })
    } catch (e) {
      setError(e instanceof CoinStatsError ? e.message : 'Something went wrong. Try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className={styles.gate}>
      <div className={styles.inner}>
        ...
        <motion.h1 custom={1} variants={rise} initial="hidden" animate="show" className={styles.title}>
          Your crypto, mapped to <span className="text-grad-gold">the entire world</span>.
        </motion.h1>

        <motion.p custom={2} variants={rise} initial="hidden" animate="show" className={styles.sub}>
          Connect your CoinStats portfolio and a real, interactive map of every country lights up —
          showing whether your stack makes you a <strong>king</strong> or leaves you on the street,
          country by country.
        </motion.p>

        <motion.div custom={3} variants={rise} initial="hidden" animate="show" className={styles.cardWrap}>
          <GlassPanel glow="var(--gold)" innerClassName={styles.card}>
            <label className={styles.fieldLabel} htmlFor="apikey">
              ...
              <a className={styles.getKey} href="https://coinstats.app/api/" target="_blank" rel="noreferrer">
            ...
            <button className={styles.advToggle} onClick={() => setAdvanced((v) => !v)}>
              <ChevronDown
                className={`lucide ${styles.chev} ${advanced ? styles.chevOpen : ''}`}
                size={15}
              />
                ...
                >
                  <div className={styles.advInner}>
                    <div className={styles.advField}>
                      <span className={styles.smallLabel}>Share token (optional)</span>
                      <input
                    ...
                    <div className={styles.advField}>
                      <span className={styles.smallLabel}>Endpoint base URL</span>
                      <input
                        className={styles.smallInput}
                        value={baseUrl}
                        spellCheck={false}
            ...
            <div className={styles.privacy}>
              ...
              Your key is used only in your browser to call CoinStats directly. Nothing is stored.
            </div>
          </GlassPanel>
          ...
          <div className={styles.demo}>
            ...
            <div className={styles.demoRow}>
              ...
              <div className={styles.demoChips}>
                ...
                {DEMO_PRESETS.map((p) => (
                  <button key={p.value} className={styles.demoChip} onClick={() => enterDemo(p.value)}>
                    {p.label}
                  </button>
                ))}
              </div>
      ...
      <div className={styles.footer}>
        {currencySymbol(currency)} · 240+ countries · realistic cost-of-living model
      </div>
    </div>
  )
}
